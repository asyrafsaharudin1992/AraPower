...
export default function App() {
  ...
  const handleAuthSuccess = (userData: Staff) => {
    const enriched = { ...userData, is_approved: userData.is_approved !== undefined ? userData.is_approved : 1 };
    localStorage.setItem('currentUser', JSON.stringify(enriched));
    setCurrentUser(enriched);
    setShowWelcome(true);
    if (userData.role === 'admin' || userData.role === 'manager') setActiveTab('admin');
    else if (userData.role === 'receptionist') setActiveTab('receptionist');
